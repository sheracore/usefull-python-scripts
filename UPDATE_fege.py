...
class Update:
    ...
    def get_file_data(self):
        ...
        for row in data[1:]:
            ...
            subtype = row[7]
            port_level = row[9]
            cursor.execute("select distinct(resource_name) from mtn_transmission_mapping where device_name = '%s' and type='undefined'" %(device_name))
...
if __name__ == '__main__':
    postgres_txt = "host='***' dbname='****' user='****' password='****'"
